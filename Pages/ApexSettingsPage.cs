...
namespace VibranceHud.Pages
{
    ...
    public sealed class ApexSettingsPage : GlowPage
    {
        ...
        public ApexSettingsPage(DetectedGame game, Action onBack)
        {
            _game = game;
            ...
            var launch = SettingsPage.FlatButton("▶  Launch Apex", Pad + CardW - 150, y + 24, 150);
        ...
        private void Apply()
        {
            if (ApexSettingsService.IsApexRunning())
            {
                var proceed = MessageBox.Show(
                    "Apex is running and may overwrite these changes when it exits.\n\n" +
                    "Apply anyway? (Recommended: close Apex first.)",
